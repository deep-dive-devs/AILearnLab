...
const Goals = () => {
  ...
  const [userData, setUserData] = useState(
    JSON.parse(searchParams.get("userData"))
  );
    ...
    <div className="flex flex-col px-4">
      <div className="flex justify-between flex-grow mb-2">
        ...
        <div className="flex flex-col p-4 w-2/5 ml-2 bg-slate-400 rounded-md">
          <div className="text-3xl mx-auto font-extrabold">Recent Goals</div>
        </div>
      </div>
      {userData && (
        <AllGoals key={userId} userData={userData} deleteGoal={deleteGoal} />
      )}
    </div>
  );
};
